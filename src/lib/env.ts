...
const startupEnvSchema = z.object({
  ...
  OPENCLAW_GATEWAY_AUTH_MODE: z.enum(["token", "password"]).optional(),
...
export function getStartupEnv() {
  if (!cachedStartupEnv) {
    cachedStartupEnv = startupEnvSchema.parse({
      ...
      OPENCLAW_GATEWAY_AUTH_MODE: emptyToUndefined(process.env.OPENCLAW_GATEWAY_AUTH_MODE),
      OPENCLAW_GATEWAY_TOKEN: emptyToUndefined(process.env.OPENCLAW_GATEWAY_TOKEN),
      OPENCLAW_GATEWAY_PASSWORD: emptyToUndefined(process.env.OPENCLAW_GATEWAY_PASSWORD),
...
export function getStartupEnvDiagnostics() {
  ...
  return {
    ...
    fallbackGatewayAuthMode: env.OPENCLAW_GATEWAY_AUTH_MODE ?? null,
    fallbackGatewayTokenConfigured: Boolean(env.OPENCLAW_GATEWAY_TOKEN),
    fallbackGatewayPasswordConfigured: Boolean(env.OPENCLAW_GATEWAY_PASSWORD),
